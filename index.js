...
async function starts() {
	...
	client.on('chat-update'), async (mek) => {
		try {
			...
			mess = {
				wait: '⌛  aguarde sua figurinha ser feita ⌛ ,
				...
				only: {
					...
					ownerG: '❌  Este comando só pode ser usado em grupos!❌',
					ownerB: '❌ Este comando só pode ser usado em grupos! ❌',
					admin: '❌ Este comando só pode ser usado em grupos! ❌',
					Badmin: '❌ Este comando só pode ser usado em grupos! ❌'
				}
			}
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*guhh bot: ${me.name}\n*wa.me/5519994414618t : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*total de contatos bloqueados* : ${blocked.length}\n*The bot is active on* : ${kyun(uptime)}`
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ GFalhou, no momento da conversão ❌ ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`enviar fotos com legendas ${prefix}adesivo ou tag de imagem que foi enviado`)
					}
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'onde está o código da linguagem tio'?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'cadê o texto tio', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('Texto muito grande tio')
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Cadê o url bro?')
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('o que você quer eescrever')
					teks = body.slice(7)
					reply(mess.wait)
				...
				case 'url2img':
					...
					if (args.length < 2) return reply('Cadê o url tio'?')
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Berhasil Promote @${mentioned[0].split('@')[0]} como admin do grupo!`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Berhasil Demote @${mentioned[0].split('@')[0]}  tornou-se membro do grupo!`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('quem quer adicionar tio'?')
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('marque o alvo que você quer chutar')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ adesivo de resposta ❌')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falhou, ao converter o adesivo para a imagem ❌')
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('Modo simi ativo')
						samih.push(from)
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Ativar com sucesso o modo simi nestegrupo ✔️')
					} else {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Desativado com sucesso o recurso de boas-vindas neste grupo')
					}
                                      break
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
